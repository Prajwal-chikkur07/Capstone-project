...
async def handle_text_translation(request: TranslationRequest):
    try:
        ...
        # Return original text instead of 500 — keeps continuous listening working
...
class AdvancedTranslateRequest(BaseModel):
    text: str
    target_language: str
    glossary: dict | None = None  # { "term": "translation" }

@app.post("/api/advanced-translate")
async def handle_advanced_translation(request: AdvancedTranslateRequest):
    """
    Advanced Gemini-powered translation with:
    - Context analysis (domain, tone, slang detection)
    - Jargon preservation via optional glossary
    - Slang/idiom normalization
    - Confidence scoring
    Returns full analysis + final_translation.
    """
    if not request.text or not request.text.strip():
        raise HTTPException(status_code=400, detail="Text cannot be empty")
    try:
        from services.gemini_client import advanced_translate
